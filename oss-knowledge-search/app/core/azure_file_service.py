...
class AzureFileService:
    ...
    async def upload_file(
        ...
        try:
            ...
            file_client.upload_file(file_content, overwrite=True)
    ...
    def generate_download_url(self, file_path: str) -> Optional[str]:
        ...
        Returns:
            Proxy endpoint URL pointing to embedding server
        """
        # Lazy initialization on first use
        self._ensure_initialized()
        ...
        try:
            ...
            # Get the embedding server URL from settings or use default
            base_url = getattr(settings, 'EMBEDDING_SERVER_URL', 'http://localhost:8000')
            # Use correct path with /qdrant prefix
            download_url = f"{base_url}/qdrant/collections/files/download?path={encoded_path}"
            
            logger.info(f"Generated proxy download URL for {file_path}")
            return download_url
