                ...
                >
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative search-dropdown-container">
                            <button
                                :disabled="isLoading"
                                class="px-3 py-2 text-sm disabled:opacity-50 transition-all duration-200 rounded-lg border font-medium flex items-center gap-1 text-gray-600 hover:text-gray-800 hover:bg-gray-100 border-gray-200"
                                title="검색 옵션"
                            >
                                검색
                                <svg
...
const handleSend = async () => {
    ...
    console.log("MockChatInput - 원본 메시지:", rawMessage);
    console.log("MockChatInput - 전송할 메시지:", messageContent);
    console.log("MockChatInput - 메시지 길이:", messageContent.length);

    // 입력창 즉시 초기화 (Vue 반응형 데이터와 DOM 모두 초기화)
    message.value = "";
    ...
    if (textareaRef.value) {
        textareaRef.value.value = "";
        textareaRef.value.focus(); // 포커스 유지
    }
...
// 중지 함수 (실제로는 시뮬레이션을 중지할 수 없지만 UI 일관성을 위해)
const stopGeneration = () => {
    // Mock chat에서는 시뮬레이션을 중지할 수 없지만 UI 일관성을 위해 함수만 정의
