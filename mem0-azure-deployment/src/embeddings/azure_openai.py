...
class AzureOpenAIEmbedding(EmbeddingBase):
    ...
    def embed(self, text, memory_action: Optional[Literal["add", "search", "update"]] = None):
        ...
        """
        text = text.replace("\n", " ")
        # Use dimensions parameter if embedding_dims is configured
        embedding_params = {"input": [text], "model": self.config.model}
        if self.config.embedding_dims:
            embedding_params["dimensions"] = self.config.embedding_dims
